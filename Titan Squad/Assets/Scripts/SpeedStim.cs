...
public class SpeedStim : Item
{
    ...
    {
        StartCoroutine(useSpeedStim());
    }

    IEnumerator useSpeedStim()
    {
        PlayerUnit player = transform.parent.GetComponent<PlayerUnit>();

        MapBehavior.instance.setColor('y');
        ...
        shader.SetColor("_OverlayColor", Color.yellow);
